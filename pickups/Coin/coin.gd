...
func _ready() -> void:
	...
	if GameData.level_progress.has(path_folder) and GameData.level_progress[path_folder].has(current_level):
...
func _on_body_entered(body: Node2D) -> void:
	if body is Player:
		...
		# mark as collected
		GameData.level_progress[pathToItself.get_slice("/", 1)][get_tree().current_scene.scene_file_path.get_file()][coinNumber] = true
